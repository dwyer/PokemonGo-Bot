...
class Cleanup(BaseTask):
            ...
            if len(group) > 0:
                ...
                if keep_best:
                    ...
                    if keep_best_iv >= 1:
                        ...
                        order_criteria.append('iv')

                    if keep_best_cpiv >= 1:
                        cpiv_limit = keep_best_cpiv
                        best_cpiv_pokemons = sorted(group, key=lambda x: (x['cpiv'], x['cp'], x['iv']), reverse=True)[:cpiv_limit]
                        best_pokemon_ids |= set(pokemon['pokemon_data']['id'] for pokemon in best_cpiv_pokemons)
                        order_criteria.append('cpiv')

                    if order_criteria:
                        order_criteria = ' and '.join(order_criteria)
                    else:
                        order_criteria = 'none'
                    ...
                    if transfer_pokemons:
                        ...
                        for pokemon in transfer_pokemons:
                            self.release_pokemon(pokemon_name, pokemon['cp'], pokemon['iv'], pokemon['cpiv'], pokemon['pokemon_data']['id'])
                else:
                    group = sorted(group, key=lambda x: x['cp'], reverse=True)
                    for item in group:
                        pokemon_cp = item['cp']
                        pokemon_potential = item['iv']
                        pokemon_cpiv = item['cpiv']

                        if self.should_release_pokemon(pokemon_name, pokemon_cp, pokemon_potential, pokemon_cpiv):
                            self.release_pokemon(pokemon_name, item['cp'], item['iv'], item['cpiv'], item['pokemon_data']['id'])
    ...
    def _release_pokemon_get_groups(self):
        ...
        for pokemon in inventory_dict:
            ...
            group_pokemon_cpiv = self.get_pokemon_cpiv(pokemon_data)
    ...
    def get_pokemon_cpiv(self, pokemon_data):
        return round(self.get_pokemon_potential(pokemon_data) * pokemon_data['cp_multiplier'], 2)

    def should_release_pokemon(self, pokemon_name, cp, iv, cpiv, keep_best_mode = False):
        ...
        if (keep_best_mode
            ...
            and not release_config.has_key('release_below_cpiv')):
            return True
        ...
        release_cpiv = release_config.get('release_below_cpiv', 0)
        if cpiv < release_cpiv:
            release_results['cpiv'] = True
        ...
        if logic_to_function[cp_iv_logic](*release_results.values()):
            self.emit_event(
                ...
                data={
                    ...
                    'below_iv': '%s %s CPIV < %s' % (release_iv, cp_iv_logic.upper(), release_cpiv),
                }
            )

        return logic_to_function[cp_iv_logic](*release_results.values())

    def release_pokemon(self, pokemon_name, cp, iv, cpiv, pokemon_id):
